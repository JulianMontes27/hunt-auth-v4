...
function SignInForm() {
  ...
  const handlePasskeyLogin = async () => {
    if (!email) {
      alert("Please enter your email address first to sign in with a passkey.");
      return;
    }
    ...
    try {
      ...
      if (response?.data) {
        ...
        if (
          ...
          alert(
            `Passkey login failed: ${response.error.message || "Unknown error"}. Please try again or use a different sign-in method.`
          );
        }
      }
    } catch (error) {
